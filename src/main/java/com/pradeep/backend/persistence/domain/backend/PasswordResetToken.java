package com.pradeep.backend.persistence.domain.backend;


import com.pradeep.backend.persistence.converters.LocalDateTimeAttributeConverter;
...
public class PasswordResetToken  implements Serializable {
    ...
    public PasswordResetToken(String token,User user,LocalDateTime creationDateTime,int expirationInMinutes){
        if (null == token || (null == user) || (null == creationDateTime)){
            throw new IllegalArgumentException("token , user and creation time cant be null");
        }
        if(expirationInMinutes ==0){
            LOG.warn("the token expiration length is zero.Assigning the default value {}",DEFAULT_TOKEN_LENGTH_IN_MINUTES);
