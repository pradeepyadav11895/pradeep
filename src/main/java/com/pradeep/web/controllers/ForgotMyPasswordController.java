...
public class ForgotMyPasswordController {
    ...
    public String forgotPasswordPost(HttpServletRequest request,
        ...
        if(null == passwordResetToken){
            LOG.warn("Coulidnot find a password resret token for email {} ",email);
        }else {
            ...
            mailMessage.setSubject("[Pradeep ]: How to Reset Your Password");
    ...
    public String changeUserPasswordGet(@RequestParam("id") long id,
        ...
        if(null == passwordResetToken){
            LOG.warn(" A token Could not found with value {} ",token);
            model.addAttribute(PASSWORD_RESET_ATRIBUTE_NAME,"false");
        ...
        User user=passwordResetToken.getUser();
     if(user.getId() != id){
         LOG.warn("This user id {} passed as paramter does not match the user id {} associated  with the token {}",id,user.getId(),token);
         model.addAttribute(PASSWORD_RESET_ATRIBUTE_NAME,"false");
     ...
     //ok to proceed we auto -authenticate the user so thaat Post request we can check if the user
        // is authenticated
    ...
    public String changePasswordPost(@RequestParam("principal_id") long userId,
        ...
        if(null == authentication){
            LOG.error("An unauthenticated user tried to invoke the reset password ");
            model.addAttribute(PASSWORD_RESET_ATRIBUTE_NAME,"false");
        ...
        LOG.info("Password successfully updated for the user {} ",user.getUsername());
